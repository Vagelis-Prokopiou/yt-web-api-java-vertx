...
//		UW = DatabindCodec.mapper().writerFor(new TypeReference<Stream<User>>() {});
		UW = DatabindCodec.mapper().writerFor(new TypeReference<User[]>() {
		});
	...
	void handle(HttpServerRequest req) {
		...
		try {
			UW.writeValue(ByteBufJsonGenerator.allocate(buffer.byteBuf().clear(), FEATURES), arr);
